...
If an area's slug collides with a reserved status directory name (`open`, `pending`, `in-progress`, `done`, `unknown`), `create` and `update --area` reject it. If existing data already has this collision, the issue stays readable by `list` and `show`, `migrate` skips it with a warning, and `validate` reports it as an error without auto-correcting. Commands that relocate an issue (`done`, `pending`, `in-progress`, `reopen`, and `update` without `--area`) treat a colliding area as no area at all and place the file at the flat `issues/<status>/` with a warning, the same fallback used for unparseable frontmatter — renga never builds the directory layout its own `create` refuses to produce.

An area slug that merely *looks* like an issue ID is fine: area `2024 Q1` slugs to `2024-q1` and works normally. An area directory is told apart from a directory-based issue by its shape — a direct child of the issues root that holds status subdirectories — not by its name.
